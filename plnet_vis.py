...
def generator_model(input_img):
    ...
    y = Conv2D(3, (3, 3), activation=None, padding='same')(x)
...
def full_model(summary = True):
    ...
    generator, vis = generator_model(input_img)
    model = Model(input=input_img, output=[generator, vis], name='architect')
    model.summary()
    return model
...
def train_faces(weights=None):
    ...
    model.compile(loss=[pixel_loss, pixel_loss], loss_weights=[1, 0], optimizer=optim)
    ...
    for epoch in range(num_epochs):
        ...
        model.save_weights("vis_weights_%d" % epoch)
...
def test(image, weights, save_name):
    ...
    cv2.imwrite(save_name, result[0][0]*255)
    cv2.imwrite('vgg1.png', result[1][0][:,:,0]*255)
    cv2.imwrite('vgg2.png', result[1][0][:,:,1]*255)
    cv2.imwrite('vgg3.png', result[1][0][:,:,2]*255)

if __name__ == "__main__":
    # train_faces()
    # test("datasets/full_test/A/20_0134.png", "weights_2", "test_vis.png")
    test("datasets/full_test/A/20_0134_s048.png", "weights_2", "test_vis.png")
